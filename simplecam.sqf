...
// --- CONFIGURATION ---
#define MOD_NAME "[Bro] Simple Cinematic Camera"
...
_ehIds pushBack (addMissionEventHandler ["EachFrame", {
	...
	if (["K_S_Fst", false] call (_d get "fnc_CheckKey")) then { _spdDes = _spdDes * 1.02; }; 
	if (["K_S_Slw", false] call (_d get "fnc_CheckKey")) then { _spdDes = _spdDes * 0.98; }; 
	...
	if (["K_M_F", false] call (_d get "fnc_CheckKey")) then { _moveVec = _moveVec vectorAdd _fwdRef; }; 
	if (["K_M_B", false] call (_d get "fnc_CheckKey")) then { _moveVec = _moveVec vectorDiff _fwdRef; }; 
	if (["K_M_R", false] call (_d get "fnc_CheckKey")) then { _moveVec = _moveVec vectorAdd _vecRightH; }; 
	if (["K_M_L", false] call (_d get "fnc_CheckKey")) then { _moveVec = _moveVec vectorDiff _vecRightH; }; 
	if (["K_M_U", false] call (_d get "fnc_CheckKey")) then { _moveVec = _moveVec vectorAdd [0,0,1]; }; 
	if (["K_M_D", false] call (_d get "fnc_CheckKey")) then { _moveVec = _moveVec vectorAdd [0,0,-1]; }; 
	...
	if (diag_tickTime > (_d get "NotifyEnd")) then { (_d get "Notify") ctrlShow false; };
